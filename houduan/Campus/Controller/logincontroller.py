...
from Models.Models import Account, Administrator, Ban
...
def login():
    ...
    existing_user = Ban.query.filter_by(j_username=j_username).first()
    if existing_user:
        return jsonify({'message': 'you have been blocked'})#, 400
...
def register():
    ...
    j_password = data.get('j_password')

    if not j_username or not j_password:
        return jsonify({'message': 'j_username and password are required'})

    existing_user = Account.query.filter_by(j_username=j_username).first()
    if existing_user:
        return jsonify({'message': 'j_username already exists'})

    new_user = Account(j_username=j_username, j_password=j_password)
