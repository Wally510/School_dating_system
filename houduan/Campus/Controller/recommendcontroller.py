...
def recommend_users():
    ...
    try:
        ...
        if hobbies is None or hobbies == '':
            return jsonify(0)
        # 将爱好拆分为列表
        user_hobbies = [h.strip() for h in hobbies.split(',')if h.strip()!='']
        # user_hobbies是一个列表，包含了用户的爱好，通过将爱好字符串拆分后得到

        # 获取所有用户的爱好
        all_users = []
        all_users_query = text(
            "SELECT j_username, hobbies FROM account WHERE j_username != :j_username and hobbies is not null and hobbies !=''")
        ...
        for user in all_users:
            # if user['j_username'] =='666666666666':
            #     target_hobbies = [h.strip() for h in user['hobbies'].split(',')if h.strip()!='']
